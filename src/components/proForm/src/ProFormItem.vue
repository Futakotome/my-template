<template>
  <el-form-item :rules="getRules" :prop="formElement.includes(widget.type) ? widget.model : undefined"
                :label-width="widget.options.hiddenLabel ? '0' : labelWidth" :class="widget.options.className">
    <template #label>
    ...
    <el-form-item v-if="widget.type === 'blank'" :label-width="widget.options.hiddenLabel ? '0' : labelWidth"
                  :prop="widget.model" :key="widget.key" v-show="!widget.hidden">
    ...
    <template v-if="widget.type === 'select'">
      <el-select
          ...
          v-on="_$listeners"
      >
        <el-option v-for="(item, index) in optionsList" :key="index" :value="item.value"
    ...
    <template v-if="widget.type === 'radio'">
      <el-radio-group v-on="_$listeners" v-model="models[widget.model]" :style="{ width: widget.options.width }"
                      :disabled="readOnly || widget.options.disabled">
        <el-radio :style="{ display: widget.options.inline ? 'inline-block' : 'block' }" :label="item.value"
    ...
    <template v-if="widget.type === 'checkbox'">
      <el-checkbox-group v-on="_$listeners" v-model="models[widget.model]" :style="{ width: widget.options.width }">
        ...
        <el-checkbox-button v-else :style="{ display: widget.options.inline ? 'inline-block' : 'block' }"
                            :label="item.value" v-for="(item, index) in optionsList" :key="item.value + index">
          {{ widget.options.showLabel ? item.label : item.value }}
        </el-checkbox-button>
      </el-checkbox-group>
    </template>
    ...
    <template v-if="widget.type === 'rate'">
      <el-rate v-on="_$listeners" v-model="models[widget.model]" :max="widget.options.max"
               :disabled="readOnly || widget.options.disabled" :allow-half="widget.options.allowHalf"></el-rate>
    </template>

    <template v-if="widget.type === 'color'">
      <el-color-picker v-on="_$listeners" v-model="models[widget.model]" :disabled="readOnly || widget.options.disabled"
                       :show-alpha="widget.options.showAlpha"></el-color-picker>
    </template>

    <template v-if="widget.type === 'switch'">
      <el-switch v-on="_$listeners" v-model="models[widget.model]"
                 :disabled="readOnly || widget.options.disabled"></el-switch>
    </template>
...
import { formElement } from "@/component/proTable/types/componentsConfig";
import { FileUpload } from "@/component/modules";
import SubForm from "@/component/proForm/src/components/subForm/index.vue";
