    ...
    <div class="base-table">
      ...
      >
        ...
        <template #btnBarPrevBtn>
          <!-- 批量删除按钮 -->
          <el-button v-if="view.btnDel" @click="btnDeletesOnClick" type="danger" size="small" icon="el-icon-delete">
            {{ text.multiDel }}
          </el-button>
          <!-- 添加按钮 -->
          <el-button v-if="view.btnAdd" type="primary" icon="el-icon-plus" size="small" @click.stop="btnAdd()">
            {{ text.add }}
          </el-button>
        ...
        <template #_action_header v-if="view.btnAddOnColumnHeader">
          <el-button icon="el-icon-plus" size="small" type="primary" style="color: white"
                     @click.stop="btnAdd"></el-button>
        </template>
        <!-- 表格自定义列/表头插槽,动态传入 -->
        <template :slot="slotName" slot-scope="scope"
                  v-for="slotName in Object.keys(slots).filter((key) => !['btnCustom', 'append', 'btnBarPrevBtn', 'dialogFooter'].includes(key))">
          <slot :name="slotName" :row="scope.row" :prop="scope.prop" :column="scope.column"></slot>
        </template>
        <template slot-scope="scope" slot="actionColumn">
          <!-- 操作列-添加按钮 -->
          <el-button v-if="view.actionColumnBtnAdd" icon="el-icon-plus" type="primary" size="mini"
                     @click.stop="actionColumnAdd(scope.row)">{{ text.add }}
          </el-button>
          <!-- 操作列-编辑按钮 -->
          ...
          <!-- 操作列-查看按钮 -->
          <el-button v-if="actionColumnBtnDetailVisible(scope.row)" type="primary" size="mini"
                     @click.stop="actionColumnDetail(scope.row)">{{ text.detail }}
          </el-button>

          <!-- 操作列-删除按钮 -->
          <el-button v-if="actionColumnBtnDelVisible(scope.row)" type="danger" size="mini"
                     @click.stop="actionColumnDel(scope.row)">{{ text.del }}
          </el-button>
          <!-- 自定义按钮 -->
