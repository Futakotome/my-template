  ...
  <div v-if="show">
    <el-form label-position="top" class="form" size="mini">
      ...
      <el-form-item label="大小" v-if="Object.keys(elementConfig.options).indexOf('size') >= 0">
        宽度：
        <el-input size="mini" style="width: 90px" type="number" v-model.number="elementConfig.options.size.width"></el-input>
        高度：
        <el-input size="mini" style="width: 90px" type="number" v-model.number="elementConfig.options.size.height"></el-input>
      </el-form-item>
      ...
      <template v-if="elementConfig.type == 'tabs'">
        <el-form-item label="选项卡样式">
          <el-radio-group v-model="elementConfig.options.type" size="small">
            <el-radio-button v-for="item in elementComponentConfig.tabs.type" :label="item.value" :key="item.value">
              {{ item.label }}
            </el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="选项卡位置">
          <el-radio-group v-model="elementConfig.options.position" size="small">
            <el-radio-button v-for="item in elementComponentConfig.tabs.position" :label="item.value" :key="item.value">
              {{ item.label }}
            </el-radio-button>
          </el-radio-group>
        </el-form-item>
...
import { elementComponentConfig, formElement } from './componentsConfig';
...
export default {
  ...
  computed: {
    show() {
      return this.elementConfig && Object.keys(this.elementConfig).length > 0;
    },
    // 依赖自定义数据源组件
    isRemoteComponent() {
